...
public class SearchBar extends Widget {
  ...
  public void keyEvent(KeyEvent keyEvent) {
    if (focused && keyEvent.getAction() == KeyEvent.PRESS) {
      ...
      switch (key) {
        case '\n':
          GlobalVariables.textInput = entry;
          reset();
          break;
        case '\b':
          delAtCursor();
  ...
  public void draw() {
    ...
    MainClass.UI.rect(xPosition, yPosition, promptWidth, height, radius, 0, 0, radius);
    MainClass.UI.fill((focused)? backgroundColor.brighter().getRGB(): backgroundColor.getRGB());
    MainClass.UI.rect(xPosition + promptWidth, yPosition, width - promptWidth, height, 0, radius, radius, 0);
  ...
  private void drawCursor() {
    ...
    cursorColor = MainClass.UI.lerpColor(textColor.getRGB(), backgroundColor.brighter().getRGB(), (time % (1000 * blinkDuration)) / (blinkDuration * 1000));
    MainClass.UI.stroke(cursorColor);
    if (!textLongerThanAreaSearchArea()) {
      MainClass.UI.line(xPosition + promptWidth + margin + getCursorOffsetLength(),
          yPosition + height - margin, xPosition + promptWidth + margin + getCursorOffsetLength(),
          yPosition + margin);
    } else {
      MainClass.UI.line(xPosition + width - margin
  ...
  private void drawSearch() {
    MainClass.UI.clip(xPosition + promptWidth + margin, yPosition, width - promptWidth - margin, height);
    if (entry.isEmpty()) {
      MainClass.UI.fill(240);
      MainClass.UI.text(ghost_text, xPosition + promptWidth + margin, yPosition + height);
    } else {
      if (!textLongerThanAreaSearchArea()) {
        ...
        MainClass.UI.text(entry, xPosition + width - margin, yPosition + height);
      }
    }
